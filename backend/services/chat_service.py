...
from services.pipelines import RAGPipeline, AgenticRAGPipeline
...
class ChatService:
    ...
    def __init__(self, pipeline: Optional[RAGPipeline] = None) -> None:
        ...
        Args:
            pipeline: The RAG pipeline to use for processing queries.
                     Defaults to AgenticRAGPipeline if not provided.
        """
        # Default to AgenticRAGPipeline
    ...
    def respond(
        ...
        if not self.pipeline.is_initialized:
            logger.info("Pipeline not initialized yet. Starting lazy initialization...")
            self.pipeline.startup()
