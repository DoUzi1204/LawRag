...
def _to_token_id_weights(lexical_weights: dict[str, float]) -> dict[int, float]:
    """
...
def to_qdrant_sparse(token_weights_dict):
    """
    ...
    """
    token_id_weights = _to_token_id_weights(token_weights_dict)
...
def ingest_data() -> None:
    ...
    for i in tqdm(range(0, total_docs, BATCH_SIZE), desc="Uploading Hybrid Points"):
        ...
        for doc, dense_vec, sparse_dict in zip(batch_docs, batch_dense, batch_sparse):
            ...
            sparse_vector = to_qdrant_sparse(sparse_dict)
